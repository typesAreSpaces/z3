/*++
  ...
  interpolant_kapur_cmds.h

  Abstract:
  Commands for Kapur interpolation.

  Author:

  Jose Abel Castellanos Joo (typesAreSpaces) 2020-01-20
...
class compute_interpolant_cmd : public parametric_cmd {
protected:
    ptr_vector<expr> m_targets;
public:
    compute_interpolant_cmd(char const * name = "compute-interpolant"):parametric_cmd(name) {}

    char const * get_usage() const override { return "<kapur-interpolant>"; }
...
class compute_elim_interpolant_cmd : public parametric_cmd {
protected:
    ptr_vector<expr> m_targets;
    svector<symbol>  m_symbols;
public:
    compute_elim_interpolant_cmd(char const * name = "compute-elim-interpolant"):parametric_cmd(name) {}

    char const * get_usage() const override { return "<kapur-elim-interpolant>"; }

    char const * get_main_descr() const override {
        return "get interpolant for formulas using elimination";
    }
    ...
    virtual void set_next_arg(cmd_context & ctx, symbol const & s) override { 
...
void install_interpolant_kapur_cmds(cmd_context & ctx) {
    ctx.insert(alloc(compute_interpolant_cmd));
    ctx.insert(alloc(compute_elim_interpolant_cmd));
}
